...
def parse_config():
    ...
    parser.add_argument('--momentum', type=float, default=0.99, help='momentum for ema')
    parser.add_argument('--workers', type=int, default=4, help='number of workers for dataloader')
    parser.add_argument('--num_ema_epochs', type=int, default=5, help='number of epochs to average')
...
def repeat_eval_ckpt(model, test_loader, args, eval_output_dir, logger, ckpt_dir, dist_test=False):
    ...
    ckpt_list = glob.glob(os.path.join(ckpt_dir, '*checkpoint_epoch_*.pth'))
    ckpt_list.sort(key=os.path.getmtime)
    ckpt_list = ckpt_list[-args.num_ema_epochs:]
    cur_epoch_id = int(ckpt_list[-1].split('/')[-1].split('.')[0].split('_')[-1])
    model.load_ema_params_from_files(filenames=ckpt_list, momentum=args.momentum, logger=logger, to_cpu=dist_test)
    model.cuda()
    cur_result_dir = eval_output_dir / 'ema' / cfg.DATA_CONFIG.DATA_SPLIT['test']
...
def main():
    ...
    model = build_network(model_cfg=cfg.MODEL, cfg=cfg, dataset=test_set)
    with torch.no_grad():
