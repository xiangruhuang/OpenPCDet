...
class SparseKPConv(Function):
    ...
    def forward(ctx, X, W, a):
        ...
        ctx.for_backward = ctx.save_for_backward(X, W, a)

        t1 = time.time()
        print(f'F: size={N*K*D1*D2}, time={t1-t0}, N={N}, K={K}, D1={D1}, D2={D2}')
        return Y
    ...
    def backward(ctx, grad_Y):
        ...
        t1 = time.time()
        print(f'B: size={N*K*D1*D2}, time={t1-t0}, N={N}, K={K}, D1={D1}, D2={D2}')
...
#        D1 = 3
#        D2 = 4
#        X = torch.nn.Parameter((torch.rand(N, D1)*0.1).cuda(), requires_grad=True)
#        W = torch.nn.Parameter((torch.rand(K, D2, D1)*0.1).cuda(), requires_grad=True)
#        a = torch.rand(N, K).cuda()
#        l0 = sparse_kpconv_aggr(X, W, a).square().sum() / 2.0
#        l0.backward()
#        eps = 1e-4
#        for n in range(N):
#            for i in range(D1):
#                X.data[n, i] += eps
#                l1 = sparse_kpconv_aggr(X, W, a).square().sum() / 2.0
