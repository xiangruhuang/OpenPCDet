...
class SequenceReprLearner(ReprLearnerTemplate):
    ...
    def forward(self, batch_dict):
        ...
        if self.training:
            ...
            if 'occupancy_acc' in tb_dict:
                ret_dict['occupancy_acc'] = tb_dict['occupancy_acc']
                disp_dict['occupancy_acc'] = tb_dict['occupancy_acc']
            disp_dict['Memory'] = f"{torch.cuda.max_memory_allocated() / 2**30:.4f} GB"
            tb_dict['Max Memory Allocated (in GB)'] = torch.cuda.max_memory_allocated() / 2**30
