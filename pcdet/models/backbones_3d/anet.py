...
class ANet(UNetTemplate):
    ...
    def __init__(self, runtime_cfg, model_cfg):
        ...
        for i in range(self.num_down_modules):
            ...
            conv_cfg = dict(
                ...
                MLP_CHANNELS=[cur_channel, cur_channel, cur_channel],
                NORM_CFG=self.norm_cfg,
                ACTIVATION=self.activation,
            )
            conv = EdgeConv(conv_cfg)
    ...
    def conv_up(self, skip, i, graph, conv_dict):
        ...
        identity = skip.feat
        for skip_module in self.skip_modules[i]:
            skip.feat, conv_dict = skip_module(skip, skip, graph, conv_dict)
        skip.feat = F.relu(skip.feat + identity)

        concat = EasyDict(skip.copy())
        concat.feat = torch.cat([up_feat, skip.feat], dim=-1)
        merge_feat, conv_dict = self.merge_modules[i](concat, concat, graph, conv_dict)
    ...
    def forward(self, batch_dict):
        ...
        grids5.up_feat, runtime_dict = self.up_modules[0](merge6, grids5, reversed_graphs['grid56'], runtime_dict)

        merge5, runtime_dict = self.conv_up(grids5, 1, reversed_graphs['grid55'], runtime_dict)
        grids4.up_feat, runtime_dict = self.up_modules[1](merge5, grids4, reversed_graphs['grid45'], runtime_dict)
        
        merge4, runtime_dict = self.conv_up(grids4, 2, reversed_graphs['grid44'], runtime_dict)
        grids3.up_feat, runtime_dict = self.up_modules[2](merge4, grids3, reversed_graphs['grid34'], runtime_dict)

        merge3, runtime_dict = self.conv_up(grids3, 3, reversed_graphs['grid33'], runtime_dict)
        grids2.up_feat, runtime_dict = self.up_modules[3](merge3, grids2, reversed_graphs['grid23'], runtime_dict)
        
        merge2, runtime_dict = self.conv_up(grids2, 4, reversed_graphs['grid22'], runtime_dict)
        grids1.up_feat, runtime_dict = self.up_modules[4](merge2, grids1, reversed_graphs['grid12'], runtime_dict)
