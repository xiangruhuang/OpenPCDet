...
class SST(nn.Module):
    ...
    def __init__(self, runtime_cfg, model_cfg, **kwargs):
        ...
        runtime_cfg['input_key'] = 'sst_transformer_out'
        self.build_post_processor(model_cfg.POST_PROCESSING_CFG, runtime_cfg)
    ...
    def forward(self, batch_dict):
        ...
        voxel_info = self.tokenizer(voxel_wise_dict)
        ...
        for i, block in enumerate(self.transformer_layers):
            ...
            voxel_feat = block(voxel_feat, pos_embed_list, ind_dict_list, 
        ...
        batch_dict['sst_transformer_out_feat'] = voxel_feat
        batch_dict['sst_transformer_out_bxyz'] = torch.cat([voxel_info['voxel_batch_index'][:, None],
                                                            voxel_info['voxel_xyz']], dim=-1)
