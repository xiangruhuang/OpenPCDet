...
class PointNet2(nn.Module):
    def __init__(self, runtime_cfg, model_cfg, **kwargs):
        ...
        for i, sa_channel in enumerate(sa_channels):
            sa_channel = [int(c*T) for c in sa_channel]
            sa_module = PointNetSetAbstraction(self.strides[i], num_neighbors, cur_channel, sa_channel, return_polar, num_sectors=num_sectors[i])
            max_num_points.append(max_num_points[-1] // self.strides[i])
            self.sa_modules.append(sa_module)
            channel_stack.append(cur_channel)
            cur_channel = sa_channel[-1]
    ...
    def forward(self, batch_dict):
        ...
        data_stack = []
        bxyz_feat = [point_bxyz, point_feat]
        data_stack.append(bxyz_feat)
        for i, sa_module in enumerate(self.sa_modules):
            bxyz_feat = sa_module(bxyz_feat)
            data_stack.append(bxyz_feat)
            key = f'pointnet2_sa{len(self.sa_modules)-i}_out'
            batch_dict[f'{key}_bxyz'] = bxyz_feat[0] 
            batch_dict[f'{key}_feat'] = bxyz_feat[1]

        bxyz_feat = data_stack.pop()
        for i, fp_module in enumerate(self.fp_modules):
            bxyz_feat_cur = data_stack.pop()
            bxyz_feat_cur[1] = fp_module(bxyz_feat_cur, bxyz_feat)
            bxyz_feat = bxyz_feat_cur
            key = f'pointnet2_fp{i+1}_out'
            batch_dict[f'{key}_bxyz'] = bxyz_feat[0] 
            batch_dict[f'{key}_feat'] = bxyz_feat[1]

        if self.output_key is not None:
            batch_dict[f'{self.output_key}_bxyz'] = bxyz_feat_cur[0]
            batch_dict[f'{self.output_key}_feat'] = bxyz_feat_cur[1]
