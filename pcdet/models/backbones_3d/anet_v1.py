...
class ANetV1(UNetTemplate):
    ...
    def forward(self, batch_dict):
        ...
        # computation
        points[1].feat = self.grid2point_convs[0](grids[1], points[1], graphs['grid2point1']) # [P1, 16]
        
        grids[2].feat, runtime_dict = self.conv_down_merge(points, grids, graphs, 0, runtime_dict)
        points[2].feat = self.grid2point_convs[1](grids[2], points[2], graphs['grid2point2'])
        
        #grids[3].feat = self.point2grid_convs[1](points[2], grids[3],  graphs['point2grid3'])
        grids[3].feat, runtime_dict = self.conv_down_merge(points, grids, graphs, 1, runtime_dict)
        points[3].feat = self.grid2point_convs[2](grids[3], points[3], graphs['grid2point3'])

        #grids[4].feat = self.point2grid_convs[2](points[3], grids[4],  graphs['point2grid4'])
        grids[4].feat, runtime_dict = self.conv_down_merge(points, grids, graphs, 2, runtime_dict)
        points[4].feat = self.grid2point_convs[3](grids[4], points[4], graphs['grid2point4'])

        #grids[5].feat = self.point2grid_convs[3](points[4], grids[5],  graphs['point2grid5'])
        grids[5].feat, runtime_dict = self.conv_down_merge(points, grids, graphs, 3, runtime_dict)
        points[5].feat = self.grid2point_convs[4](grids[5], points[5], graphs['grid2point5'])
        
        grids[6].feat, runtime_dict = self.conv_down_merge(points, grids, graphs, 4, runtime_dict)
        #grids[6].feat = self.point2grid_convs[4](points[5], grids[6],  graphs['point2grid6'])
